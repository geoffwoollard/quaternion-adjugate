...
def make_dets_prob(xyz,U,joint_distribution):
    ...
    YY = xyz.T[1].dot(weighted_xyz.T[1])
    YZ = xyz.T[1].dot(weighted_xyz.T[2])
    ZZ = xyz.T[2].dot(weighted_xyz.T[2])

    UX = U.T[0] @ joint_distribution.T @ xyz.T[0]
    UY = U.T[0] @ joint_distribution.T @ xyz.T[1]
    UZ = U.T[0] @ joint_distribution.T @ xyz.T[2]
    
    VX = U.T[1] @ joint_distribution.T @ xyz.T[0]
    VY = U.T[1] @ joint_distribution.T @ xyz.T[1]
    VZ = U.T[1] @ joint_distribution.T @ xyz.T[2]
...
def make_R_tilde_prob(xyz,U,joint_distribution):
    
    d1, d2, d3, d4, d5, d6, d7, d8, d9, d10 = make_dets_prob(xyz,U,joint_distribution)
...
def M_3D_pose_function_prob(xyz,U, joint_distribution):
...
def make_M_opt_rot_prob(xyz,U, joint_distribution):
    M = M_3D_pose_function_prob(xyz, U, joint_distribution)
    l,u = np.linalg.eigh(M)
    chi = M - np.identity(4)*l[3]
    adjugate = make_adjugate(chi)
    diags = [abs(adjugate[diag[0],diag[0]]) for diag in enumerate(adjugate)]
